...
namespace Kemp\FastAndLooseMoneyParser;
...
class FastAndLooseMoneyParser implements \Money\MoneyParser
{
    const DECIMAL_PATTERN = '/^(?P<sign>[-\(])?(?P<digits>0|[1-9]\d*)?\.?(?P<fraction>\d+)?(?P<endingsign>\))?$/';
    ...
    public function parse($money, $forceCurrency = null)
    {
        ...
        if ($currency->getCode() !== 'USD') {
            throw new \InvalidArgumentException('FastAndLooseMoneyParser only supports USD');
        }
        ...
        $negative = isset($matches['sign'])
            && ($matches['sign'] === '-' || ($matches['sign'] === '(' && isset($matches['endingsign']) && $matches['endingsign'] === ')'));

        if (isset($matches['sign']) && $matches['sign'] === '(' && (!isset($matches['endingsign']) || $matches['endingsign'] !== ')')) {
            throw new ParserException(sprintf(
                'Cannot parse "%s" to Money because of unmatched parenthesis',
                $decimal
            ));
        }
